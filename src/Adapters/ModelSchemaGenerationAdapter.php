    ...
    private function writeFilesFromFragmentsWithOptions(array $result, string $type, array $options): array
    {
        ...
        switch ($type) {
            ...
            case 'tests':
                ...
                $featureContent = $this->generateFeatureTestPhpFromFragment($modelName, $result);
    ...
    public function {$relationName}(): BelongsTo
    {
        ...
        if (! empty($options['has_many'])) {
            foreach ($options['has_many'] as $relation) {
                $relationName = Str::plural(mb_strtolower($relation)); // comments, posts, etc.
                $relationships .= "\n
    /**
...
";
     ...
     */
    private function generateFeatureTestPhpFromFragment(string $modelName, array $result = []): string
    {
        ...
        // Use TurboMaker stub as fallback
        $stubPath = __DIR__.'/../../stubs/test.feature.stub';
        if (file_exists($stubPath)) {
            $stubContent = file_get_contents($stubPath);

            return $this->processTestStub($stubContent, $modelName, $result);
        }
...
";
    ...
    private function generateUnitTestPhpFromFragment(array $result, string $modelName): string
    {
        ...
        // Use TurboMaker stub as fallback
        $stubPath = __DIR__.'/../../stubs/test.unit.stub';
        if (file_exists($stubPath)) {
            $stubContent = file_get_contents($stubPath);
        ...
        // Legacy hardcoded fallback
        $tableName = $result['metadata']['table_name'] ?? Str::snake(Str::plural($modelName));
...
';
    ...
    /**
     * Process test stub with comprehensive model data
     */
    private function processTestStub(string $stubContent, string $modelName, array $result): string
    {
        $tableName = $result['metadata']['table_name'] ?? Str::snake(Str::plural($modelName));
        ...
        // Generate fillable array for test assertions
        $fillableFields = [];
        if (isset($result['schema']['fields']) && ! empty($result['schema']['fields'])) {
            $fillableFields = array_keys($result['schema']['fields']);
        }

        // Create proper fillable array content (without brackets for template insertion)
        if ($fillableFields !== []) {
            $fillableContent = "'".implode("', '", $fillableFields)."'";
            $fillableArray = "['".implode("', '", $fillableFields)."']";
        } else {
            // Fallback to default fillable fields when no schema is available
            $fillableContent = "'name'";
            $fillableArray = "['name']";
        }

        // Comprehensive replacements for TurboMaker test stubs
        $replacements = [
            '{{ class }}' => $modelName,
            '{{class}}' => $modelName,
            '{{ table_name }}' => $tableName,
            ...
            '{{model_variable}}' => $modelVariable,
            '{{ fillable }}' => $fillableContent,
            '{{fillable}}' => $fillableContent,
            '{{ fillable_array }}' => $fillableArray,
            '{{fillable_array}}' => $fillableArray,
            '{{ test_feature_class }}' => $modelName.'Test',
            '{{test_feature_class}}' => $modelName.'Test',
            '{{ test_unit_class }}' => $modelName.'UnitTest',
            '{{test_unit_class}}' => $modelName.'UnitTest',
            '{{ test_api_class }}' => $modelName.'ApiTest',
            '{{test_api_class}}' => $modelName.'ApiTest',
        ];
